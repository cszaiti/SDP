...
def convert_examples_to_features(js, tokenizer, args, attack_flag):
    ...
    if js["project"] == "attack_train":
        ...
        if attack_flag == 1:  
            uid,instab=Calculated_weight(cleaned_code, tokenizer, args)
        else:
            uid,instab=0,0
        # code_ids = tokenizer.convert_tokens_to_ids(code_tokens)
        # code_tokens = [tok for tok in code_tokens if tok.strip() and tok != '\n']
        code_tokens = code_tokens[: args.block_size - 2]
        source_tokens = [tokenizer.cls_token] + code_tokens + [tokenizer.sep_token]
        source_ids = tokenizer.convert_tokens_to_ids(source_tokens)
        padding_length = args.block_size - len(source_ids)
        source_ids += [tokenizer.pad_token_id] * padding_length
        # code_tokens =  tokenizer.convert_tokens_to_ids(code_tokens)
    return InputFeatures(source_tokens, source_ids, js["idx"], js["target"],uid ,instab, code_tokens)


class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None, attack_flag=None):
####################加载一个API名称树用来匹配的################################
        # with open('/root/workspace/zlt/Defect-Detection-main/Defect-Detection-main/code/trie.pkl', 'rb') as f:
        #     trie = pickle.load(f)
############################################################################
        self.examples = []
        self.uids = []  # 添加uid列表存储
        ...
        with open(file_path) as f:
            for line in f:
                ...
                feature = convert_examples_to_features(js, tokenizer, args, attack_flag)
    ...
    # ,torch.tensor(self.examples[i].weight_V)
...
def evaluate(args, model, tokenizer, eval_when_training=False):
    ...
    attack_flag = 0
    eval_dataset = TextDataset(tokenizer, args, args.eval_data_file, attack_flag)
...
def attacker(args,attack_dataset, model, tokenizer):
    ...
    # train_data_dir = os.path.dirname(args.test_data_file)
    # attack_train_dir = os.path.join(train_data_dir, 'attack_train.jsonl')
...
def main():
    ...
    if args.do_train:
        ...
        attack_flag = 0
        train_dataset = TextDataset(tokenizer, args, args.train_data_file, attack_flag)
        if args.local_rank == 0:
