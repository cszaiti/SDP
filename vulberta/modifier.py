...
class TokenModifier(object):
    def __init__(self, classifier, loss, uids, my_tokenizer,args):
        self.cl = classifier
        self.loss = loss
        ...
        # for subtoken_idx in range(50000):
        #     subtoken = self.my_tokenizer.id_to_token(subtoken_idx)
        #     assert isinstance(subtoken, str)
        #     if subtoken in ["<s>", "<pad>", "</s>", "<unk>", "<mask>"]:
        #         continue
        ...
        for uid in uids:
            if uid not in ["<s>", "<pad>", "</s>", "<unk>", "<mask>"] and \
                uid[0] not in '0987654321' and \
                uid not in self.forbidden_uid:
                _uids.append(self.my_tokenizer.encode(uid).ids[1])
                # break
    ...
    def __gen_uid_mask_on_vocab(self, uids):
        ...
        _uids = _uids.reshape([50000, 1]).to(self.args.device)
        # return _uids        
    ...
    def rename_uid(self, source_ids,label , ori_uid_raw,n_candidate):
        ...
        ori_uid_str = str(ori_uid_raw)
        if ori_uid_str.isdigit() or (ori_uid_str and ori_uid_str[0] in '0987654321'):
            return None, None
        Gori_uid_raw = ori_uid_raw
        # Gori_uid_raw = 'Ġ' + ori_uid_raw
        Gori_uid_idx = self.my_tokenizer.encode(Gori_uid_raw).ids[1]
        # if not self.uids[Gori_uid_idx]:
        #     return None, None

        grad = self.cl.grad(source_ids, label)
        grad = grad[Gori_uid_idx].reshape([1, 768])
        ...
        for Gnew_uid_idx in Gnew_uid_cand:
            ...
            #     continue
            Gnew_uid_raw = self.my_tokenizer.id_to_token(int(Gnew_uid_idx))
    ...
    def rename_uid_random(self, source_ids, ori_uid, keys):
        ...
        ori_uid_str = str(ori_uid)
        if ori_uid_str.isdigit() or (ori_uid_str and ori_uid_str[0] in '0987654321'):
            return None, None
        ori_uid = self.my_tokenizer.encode(ori_uid).ids[1]
...
class InsModifier(object):
    ...
    def __init__(self, classifier,  my_tokenizer,poses=None):
        ...
        for insert in inserts:
            tokens = self.my_tokenizer.encode(insert, add_special_tokens=False).ids
            self.inserts.append(tokens)
